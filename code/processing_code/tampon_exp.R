...
sewerline_data = plyr::ddply(sewerline_data,.(date, sample, target),plyr::summarize, ave_copies = mean(copy_num_tampon)) 
...
sewerline_data %>% filter(site_id != "Y") %>% ggplot(aes(x = date, y = log_copies, fill = target)) +
